...
<template>
  ...
  >
    ...
    >
      <button
        v-for="tab in ([['current', 'Agora'], ['forecast', 'Previsão']] as const)"
        ...
        class="min-h-11 rounded-xl px-4 py-2 text-sm font-semibold transition"
        :class="modelValue === tab[0] ? 'bg-white/20 shadow-sm' : 'opacity-65 hover:opacity-100'"
        @click="select(tab[0])"
      >
        {{ tab[1] }}
      </button>
